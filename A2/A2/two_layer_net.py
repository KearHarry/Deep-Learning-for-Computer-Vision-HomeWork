...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    scores = None
    ############################################################################
    # TODO: 执行前向传递，计算输入的类分数。                                   #
    # 将结果存储在 scores 变量中，该变量应为形状 (N, C) 的张量。               #
    ############################################################################
    # Replace "pass" statement with your code
    hidden = X.mm(W1).add(b1)  # (N, H)
    hidden = hidden.clamp(min=0)  # ReLU
    scores = hidden.mm(W2).add(b2)  # (N, C)
...
def nn_forward_backward(
    ...
    loss = None
    ############################################################################
    # TODO: 根据 nn_forward_pass 的结果计算损失。                              #
    # 这应该包括数据损失和 W1、W2 的 L2 正则化。                               #
    # 将结果存储在变量 loss 中，应该是一个标量。                               #
    # 使用 Softmax 分类器损失。                                                #
    # 当您实现 W 的正则化时，请不要将正则化项乘以 1/2（无系数）。              #
    # 如果不小心，很容易遇到数值不稳定性                                       #
    # (查看 http://cs231n.github.io/linear-classify/ 中的数值稳定性)。         #
    ############################################################################
    # Replace "pass" statement with your code
    ...
    probs = log_probs.exp()
    N = X.shape[0]
    loss = -log_probs[torch.arange(N, device=X.device), y].sum() / N
    ...
    grads = {}
    ###########################################################################
    # TODO: 计算反向传播，计算权重和偏差的导数。                               #
    # 将结果存储在 grads 字典中。                                              #
    # 例如，grads['W1'] 应存储 W1 上的梯度，并且大小相同的张量。               #
    ###########################################################################
    # Replace "pass" statement with your code
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        #########################################################################
        # TODO: 使用 grads 字典中的梯度更新网络的参数（存储在字典 self.params 中）    #
        # 使用随机梯度下降。您需要使用上面定义的 grads 字典中存储的梯度。             #
        #########################################################################
        # Replace "pass" statement with your code
...
def nn_get_search_params():
    ...
    ###########################################################################
    # TODO: 添加您自己的超参数列表。                                          #
    # 这应该类似于您用于 SVM 的超参数，但您可能需要选择不同的超参数           #
    # 以在 softmax 分类器中获得良好的性能。                                   #
    ###########################################################################
    # Replace "pass" statement with your code
...
def find_best_net(
    ...
    #############################################################################
    # TODO: 使用验证集调整超参数。将训练最好的模型存储在 best_net 中。          #
    #                                                                           #
    # 为了帮助调试网络，使用类似于上面的可视化可能会有所帮助；                  #
    # 这些可视化与我们在上面看到的调整不当的网络会有显着的质的差异。            #
    #                                                                           #
    # 手动调整超参数可能很有趣，但你会发现像我们在之前的练习中所做的那样        #
    # 编写代码自动扫描可能的超参数组合很有用。                                  #
    #############################################################################
    # Replace "pass" statement with your code
            ...
            for reg in regularization_strengths:
                for decay in learning_rate_decays:
                    ...
                    if val_acc > best_val_acc:
                        ...
                        best_net = model
                        best_stat = stats
                        print(f"New best val_acc: {best_val_acc} (lr {lr}, hidden {hidden_size}, reg {reg}, decay {decay})")
