...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        # Use torch.nn.functional.pad to pad x
        ...
        # 3. Convolution (Naive loop implementation)
        for n in range(N):  # Iterate over samples
            for f in range(F):  # Iterate over filters
                for i in range(H_out):  # Iterate over output height
                    for j in range(W_out):  # Iterate over output width
                        # Determine current window on padded input
                        # Start position
                        h_start = i * stride
                        w_start = j * stride
                        # End position
    ...
    def backward(dout, cache):
        ...
        # Pad input x to match gradient calculation indices
        x_pad = torch.nn.functional.pad(x, (pad, pad, pad, pad), mode='constant', value=0)
            ...
            for f in range(F):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        # 1. Gradient w.r.t bias: sum of dout over N, H_out, W_out
                        db[f] += dout_val

                        # 2. Gradient w.r.t weights: convolution of input with dout
                        dw[f] += x_pad[n, :, h_start:h_end, w_start:w_end] * dout_val

                        # 3. Gradient w.r.t padded input: convolution of dout with weights (full convolution)
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        # MaxPool 也是一种降采样操作。
        # 它的作用是在输入特征图的局部区域内取最大值，从而减小特征图的尺寸，
        # 并使特征对微小的平移具有不变性。
            ...
            for c in range(C):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        # 将上游梯度 dout 传回最大值所在位置
                        # 注意：如果有多个最大值，梯度会平均分配给它们，或者通常在这个实现中，
                        # 简单的加法意味着如果同一个 x 元素参与了多次池化（这在 stride < size 时可能发生）
                        # 或者是同一个窗口内有多个最大值，PyTorch 的 tensor 操作会自动处理广播
                        # 这里我们利用 mask 将 dout 只加到 max_val 对应的位置
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W1'] = weight_scale * torch.randn(F, C, filter_size, filter_size, dtype=dtype, device=device)
        self.params['b1'] = torch.zeros(F, dtype=dtype, device=device)
        # Pooling layer params: None
        # Hidden layer params
        # Pooling halves dimensions, so input to FC is F * (H/2) * (W/2)
    ...
    def loss(self, X, y=None):
        ...
        relu_out1, relu_cache1 = ReLU.forward(conv_out)
        pool_out, pool_cache = MaxPool.forward(relu_out1, pool_param)
        # 2. FC + ReLU
        # Flatten pool output
        N = X.shape[0]
        ...
        # 5. Pool + ReLU + Conv backward
        dpool_out = dpool_out_flat.view(pool_out.shape)
        drelu_out1 = MaxPool.backward(dpool_out, pool_cache)
        dconv_out = ReLU.backward(drelu_out1, relu_cache1)
        dX, dW1, db1 = Conv.backward(dconv_out, conv_cache)
        # 6. Store gradients
...
class DeepConvNet(object):
    ...
    padding 1 to preserve the feature map size, and all pooling layers be
    ...
    def __init__(self,
        ...
        for i in range(len(num_filters)):
            # Conv layer weight and bias
            if weight_scale == 'kaiming':
                self.params['W%d' % (i + 1)] = kaiming_initializer(
                    C, F[i], K=3, relu=True, device=device, dtype=dtype)
            else:
                self.params['W%d' % (i + 1)] = weight_scale * torch.randn(
                    F[i], C, 3, 3, dtype=dtype, device=device)

            self.params['b%d' % (i + 1)] = torch.zeros(F[i], dtype=dtype, device=device)

            # BN params
            if batchnorm:
                self.params['gamma%d' % (i + 1)] = torch.ones(F[i], dtype=dtype, device=device)
                self.params['beta%d' % (i + 1)] = torch.zeros(F[i], dtype=dtype, device=device)
            # Update input C for next layer
            C = F[i]
            # Max pool halves dimensions
            if i in max_pools:
                H //= 2
                W //= 2
        # Output layer
        if weight_scale == 'kaiming':
            self.params['W%d' % self.num_layers] = kaiming_initializer(
                C * H * W, num_classes, relu=False, device=device, dtype=dtype)
        else:
            self.params['W%d' % self.num_layers] = weight_scale * torch.randn(
                C * H * W, num_classes, dtype=dtype, device=device)

        self.params['b%d' % self.num_layers] = torch.zeros(
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.batchnorm:
                ...
                out, cache = Conv_BatchNorm_ReLU.forward(out, W, b, gamma,
                                                          beta, conv_param, self.bn_params[i])
            else:
        ...
        # 4. Backward pass for each macro layer
        for i in range(self.num_layers - 2, -1, -1):
            ...
            if i in self.max_pools:
                ...
                conv_cache = cache
            if self.batchnorm:
                dout, dW, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dout, conv_cache)
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        dx_norm = dout * gamma
        dvar = torch.sum(dx_norm * (x - sample_mean) * -0.5 * (sample_var + eps)**(-1.5), dim=0)
        dmean = torch.sum(dx_norm * -1.0 / torch.sqrt(sample_var + eps), dim=0) + \
                dvar * torch.mean(-2.0 * (x - sample_mean), dim=0)

        dx = dx_norm / torch.sqrt(sample_var + eps) + \
             dvar * 2.0 * (x - sample_mean) / N + \
             dmean / N
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        # Gradient with respect to beta: sum over the batch dimension
        dbeta = torch.sum(dout, dim=0)

        # Gradient with respect to gamma: sum of (dout * x_norm) over batch
        dgamma = torch.sum(dout * x_norm, dim=0)

        # Gradient with respect to x_norm
        dx_norm = dout * gamma

        # Gradient with respect to x using the simplified formula:
        # dx = (1/N * sqrt(var + eps)) * (N * dx_norm - sum(dx_norm) - x_norm * sum(dx_norm * x_norm))
        # This formula combines the gradients from normalization, mean, and variance.

        # English Explanation of the implementation idea:
        # The backward pass for Batch Normalization involves computing gradients for the input x.
        # Through derivation using the chain rule, we can simplify the expression for dx.
        # The term 1/sqrt(var + eps) comes from the normalization.
        # The term (N * dx_norm) represents the direct gradient flow.
        # The term sum(dx_norm) corrects for the mean subtraction.
        # The term x_norm * sum(dx_norm * x_norm) corrects for the variance division.
        # The factor 1/N averages these corrections over the batch.

        std_inv = 1.0 / torch.sqrt(var + eps)
        dX_norm_sum = torch.sum(dx_norm, dim=0)
        dX_norm_x_norm_sum = torch.sum(dx_norm * x_norm, dim=0)

        dx = (1.0 / N) * std_inv * (N * dx_norm - dX_norm_sum - x_norm * dX_norm_x_norm_sum)
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        ################################################################
        # Spatial Batch Normalization Explanation:
        # Normal Batch Normalization works on shape (N, D).
        # Convolutional output has shape (N, C, H, W).
        # We want to normalize each channel across all images and all spatial locations.
        # So we treat each channel as a feature, and the effective batch size becomes N*H*W.
        # Effectively we reshape (N, C, H, W) -> (N*H*W, C)

        N, C, H, W = x.shape
        # Permute to (N, H, W, C) then flatten to (N*H*W, C)
        x_flat = x.permute(0, 2, 3, 1).contiguous().view(-1, C)

        # Apply standard BatchNorm
        out_flat, cache = BatchNorm.forward(x_flat, gamma, beta, bn_param)

        # Reshape back to (N, H, W, C) then permute to (N, C, H, W)
    ...
    def backward(dout, cache):
        ...
        #################################################################
        N, C, H, W = dout.shape
        dout_flat = dout.permute(0, 2, 3, 1).contiguous().view(-1, C)
        dx_flat, dgamma, dbeta = BatchNorm.backward(dout_flat, cache)
        dx = dx_flat.view(N, H, W, C).permute(0, 3, 1, 2).contiguous()
        ##################################################################
        #                       END OF YOUR CODE                         #
        ##################################################################
