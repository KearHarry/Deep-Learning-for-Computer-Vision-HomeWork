...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        X = X.to(self.params['W1'].dtype)

        # 第一层前向传播：Linear -> ReLU
        # 使用 Linear_ReLU.forward 函数
        # 输入 X, 权重 W1, 偏置 b1
        # 输出 a1 (第一层激活值), cache1 (保存用于反向传播的中间值)
        a1, cache1 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])

        # 第二层前向传播：Linear (作为输出层，通常后面接 Softmax Loss)
        # 使用 Linear.forward 函数
        # 输入 a1, 权重 W2, 偏置 b2
        # 输出 scores (类别得分), cache2 (保存用于反向传播的中间值)
        ...
        # 1. 计算 Softmax 数据损失和关于分数的梯度
        # softmax_loss 函数返回数据损失 data_loss 和关于 scores 的梯度 dscores
        data_loss, dscores = softmax_loss(scores, y)

        # 2. 计算正则化损失 (L2 Regularization)
        # 公式: reg * (sum(W1^2) + sum(W2^2))
        # 注意: 根据作业提示，这里不乘 0.5
        ...
        # 第二层反向传播 (Linear)
        # 输入: dscores (关于输出分数的梯度)
        # 输出: da1 (关于隐藏层的梯度), dW2 (关于 W2 的梯度), db2 (关于 b2 的梯度)
        ...
        # 第一层反向传播 (Linear -> ReLU)
        # 输入: da1 (来自上一层的梯度)
        # 输出: dx (关于输入的梯度), dW1 (关于 W1 的梯度), db1 (关于 b1 的梯度)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            # i counts from 0 to num_layers - 1
            # keys are W1, W2, ... so use i + 1
            layer_idx = i + 1
            in_dim = dims[i]
            out_dim = dims[i + 1]
            self.params['W' + str(layer_idx)] = weight_scale * torch.randn(in_dim, out_dim, dtype=dtype, device=device)
            self.params['b' + str(layer_idx)] = torch.zeros(out_dim, dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        ##################################################################
        # Replace "pass" statement with your code
        # 这里我们需要实现一个循环来处理任意数量的隐藏层
        # 输入 X 经过 num_layers-1 个 Linear-ReLU(-Dropout) 层，最后经过一个 Linear 层得到 scores
        out = X
        caches = []
        for i in range(1, self.num_layers):
            # 线性变换 + ReLU
            out, cache = Linear_ReLU.forward(out, self.params['W' + str(i)], self.params['b' + str(i)])
            caches.append(cache)

            # 如果使用 dropout，则在每个隐藏层后面添加 dropout 层
            if self.use_dropout:
        ...
        data_loss, dscores = softmax_loss(scores, y)
        # 2. 计算正则化损失 (L2 Regularization)
        reg_loss = 0.5 * self.reg * sum(torch.sum(self.params['W' + str(i)] ** 2) for i in range(1, self.num_layers + 1))
        # 总损失 = 数据损失 + 正则化损失
        loss = data_loss + reg_loss
...
def create_solver_instance(data_dict, dtype, device):
    ...
    # 使用 Solver 实例来训练模型
    # 我们设置学习率为 5e-2，迭代20个epoch，batch_size为100
    # 这里的 update_rule 是本文件中定义的 sgd 函数
...
def get_three_layer_network_params():
    ...
    ###############################################################
    weight_scale = 1e-1   # Initial weight scale
...
def rmsprop(w, dw, config=None):
    ...
    #    这步是为了衡量过去一段时间内梯度的平均大小。
    #    如果梯度一直很大，cache 就会很大；如果梯度很小，cache 就会很小。

    cache = decay_rate * cache + (1 - decay_rate) * (dw ** 2)

    # 2. 更新参数 (Parameter Update)
    #    w = w - learning_rate * dw / (sqrt(cache) + epsilon)
    #    这里用 dw 除以 sqrt(cache)，实现了自适应学习率。
    #    - 对于梯度较大的参数，分母大，更新步长变小，防止震荡。
    #    - 对于梯度较小的参数，分母小，更新步长变大，加速收敛。
    #    epsilon 是为了防止分母为 0 的数值稳定项。
...
def adam(w, dw, config=None):
    ...
    # 2. Update biased second raw moment estimate (RMSProp-like)
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            # Inverted Dropout 的核心思想是：在训练阶段，我们随机丢弃一些神经元的输出，并且为了保持输出的期望不变，我们将保留的神经元的输出进行缩放。
            # 1. 生成 dropout mask
            # mask 是一个与输入 x 形状相同的张量，其中每个元素以概率 p 被设置为 0（丢弃），以概率 1-p 被设置为 1/(1-p)（保留并缩放）
            ...
            ##############################################################
            # Replace "pass" statement with your code
            # 在测试阶段，我们不进行 dropout，直接返回输入 x 作为输出
            out = x
            ##############################################################
            #                      END OF YOUR CODE                      #
            ##############################################################
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
            ...
            ###########################################################
            # Replace "pass" statement with your code
            # 在训练阶段，反向传播时，我们只需要将上游梯度 dout 乘以 dropout mask 就可以了，因为丢弃的神经元对应的 mask 是 0，它们的梯度自然为 0；保留的神经元对应的 mask 是 1/(1-p)，它们的梯度会被放大以保持期望不变。
            dx = dout * mask
