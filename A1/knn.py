...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_flatten= x_train.view(num_train, -1)
    x_test_flatten = x_test.view(num_test, -1)
    x_train_square = torch.sum(x_flatten ** 2, dim=1, keepdim=True)
    x_test_square = torch.sum(x_test_flatten ** 2, dim=1, keepdim=True)
    cross_term = torch.mm(x_flatten, x_test_flatten.t())
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        y_test_pred = predict_labels(dists, self.train_labels, k=k)
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    best_k = max(k_to_accuracies, key=lambda k: (torch.tensor(k_to_accuracies[k]).mean(), -k))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
